...
UPDATE users SET firebase_uid = '33MtDnPBWCTUpJNusi4kFdqbQO93' 
WHERE email = 'sara@mhjournal.com';

UPDATE users SET firebase_uid = 'fhxgTpn3BuSe0qr0yh5rLr7Fazc2' 
WHERE email = 'imran@mhjournal.com';

UPDATE users SET firebase_uid = 'ZXStbAw3s3Mqk5sZ5HqDQu6gSW32' 
WHERE email = 'admin@mhjournal.com';

UPDATE users SET firebase_uid = '2oByEnDbaNMfRGdHpNWnVSODqOz1' 
WHERE email = 'ali@mhjournal.com';
...
SELECT email, role, date_of_birth, created_at 
FROM users 
ORDER BY created_at DESC;

-- Allow users to insert their own trigger logs
...
CREATE TABLE appointments (
  ...
  therapist_name text DEFAULT 'Dr. Imran Khan',
...
INSERT INTO therapist_patient_link (therapist_id, patient_id, is_active)
SELECT 2, user_id, true 
FROM users 
WHERE email = 'ridafaakhar@gmail.com'
ON CONFLICT DO NOTHING;

SELECT * FROM therapist_patient_link 
WHERE patient_id = (
  SELECT user_id FROM users WHERE email = 'ridafaakhar@gmail.com'
);

SELECT * FROM therapist_patient_link WHERE patient_id = 9;
...
CREATE POLICY "Allow all crisis alert operations"
ON crisis_alerts FOR ALL USING (true) WITH CHECK (true);

ALTER TABLE trigger_logs ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Allow all trigger log operations"
ON trigger_logs FOR ALL USING (true) WITH CHECK (true);

ALTER TABLE therapist_patient_link ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Allow all therapist patient link operations"
ON therapist_patient_link FOR ALL USING (true) WITH CHECK (true);

-- Allow admin to read all users
CREATE POLICY "Admin read all users" ON users
FOR SELECT USING (true);

-- Allow admin to read all journal entries  
CREATE POLICY "Admin read all trigger_logs" ON trigger_logs
FOR SELECT USING (true);

-- Allow admin to update users (for suspend/verify)
...
-- Allow admin to read therapist links
CREATE POLICY "Admin read therapist links" ON therapist_patient_link
FOR SELECT USING (true);
-- ============================================================
